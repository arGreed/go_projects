...
func (user User) IsValid() bool {
	if user.Id != 0 || user.Age < 1 || user.Age > 100 || !strings.Contains(user.Email, "@") || user.Email == "" || user.Name == "" || user.Password == "" || user.Role == "" {
		return false
	}
	return true
}
...
func CurseController() {
	...
	router.GET("/users", admAllUsers(db))
	router.GET("/courses", admAllCourses(db))
